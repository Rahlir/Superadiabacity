...
function[fig] = plot_summary_v2(data, inds)
    ...
    for ind=1:length(inds)
        ...
        pl = get_pl(identifier);
        frame = get_frame(identifier);
        info = get_info(identifier);

        pls(ind) = pl;
        
        leg = sprintf('%.0fus,n=%d,%s', pl*1e6, frame, info);
        legs(ind) = leg;
        
        x = 0 : pl/299 : pl;
        [x_ax, f_states, y2] = get_stress(data, i);
        ...
        [qs,Q] = get_Q_curves(pulse(:, :, 1), pulse(:, :, 2), pl/299, 10);
        qs_all(:, ind) = qs;
