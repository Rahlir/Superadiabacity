...
% Prepare pulse.
omega_1 = prepare_initial_pulse(nop, power, detune_factor);
delta_omega = get_tanh(nop, 25, 40, power, 15, T/nop);
detune_factor = max(delta_omega); % set max detuning of initial LZ pulse
...
for j = 1 : frame_no
    ...
    for p = 1 : rep
        ...
        if Q > 0.8
            ...
            if Q_add > max_q
                max_q = Q_add;
                best_pulse(1, :) = mod_delta;
                best_pulse(2, :) = omega_1;
            end
        end
    end
end
